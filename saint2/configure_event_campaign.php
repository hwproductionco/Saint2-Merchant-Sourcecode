    ...
    <div id="pane3" class="tab-pane">
	  ...
	  $rewards	=	 submit_cURL ($data);
       ...
       <tr>
        ...
        <?php if($parse_array_user_info['response']['user']['user_permissions']['delete_rewards'] == 'Yes'){ ?><td><a href="delete_reward.php?reward_id=<?php echo $reward['id'];?>&camp_id=<?php echo $camp_id;?>&page_id=2" onclick="return confirm('Are you sure?')"><button type="submit" class="btn-small btn-danger"><i class="icon-trash"></i> Delete</button></a></td><?php } ?>
        </tr>
        
		<?php } }}?>
        </table></div>
       <?php if($parse_array_user_info['response']['user']['user_permissions']['add_rewards'] == 'Yes'){ ?><div style="margin-top:15px;"> <button type="submit" class="btn btn-success" onclick="window.location.href='configure_event_program_reward.php?camp_id=<?php echo $camp_id;?>'"><i class="icon-plus"></i> Add</button> </div><?php } ?>
